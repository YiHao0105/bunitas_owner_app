...
class ProfileController extends GetxController with GetTickerProviderStateMixin implements GetxService {
  ...
  void onInit() {
    ...
    haveShop.value =  parser.getHaveShop().toString();
  ...
  void onPolicy() {
    Get.delete<PolicyController>(force: true);
    Get.toNamed(AppRouter.getPolicyRoute());
  }

  void onEditProfile() {
    if (type == true) {
      Get.delete<ProfileCategoriesController>(force: true);
      Get.toNamed(AppRouter.getProfileCategoriesRoute())!.then((value) {
        refresh();
      });
    } else {
      Get.delete<IndividualProfileController>(force: true);
      Get.toNamed(AppRouter.getIndividualProfileRoute())!.then((value) {
        refresh();
      });
    }
  }
