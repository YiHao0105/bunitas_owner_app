...
class ServicesCategoriesController extends GetxController implements GetxService {
  ...
  String selectedTreatmentId_Saved = '';
  ...
  void onInit() {
    ...
    selectedTreatmentId_Saved = Get.arguments[0].toString().isEmpty ? "1" : Get.arguments[0].toString();
  ...
  Future<void> getSelectedCategories() async {
    if (userType == true) {
     ...
     // var response = await parser.selectCategories({"id": parser.sharedPreferencesManager.getString('uid')});
      var response = await parser.selectCategories({"id": selectedTreatmentId_Saved});
      apiCalled = true;
      ...
      print("HelloResponse===>"+parser.sharedPreferencesManager.getString('uid').toString());
      print("HelloResponse===>"+response.bodyString.toString());
      apiCalled = true;
      if (response.statusCode == 200) {
        Map<String, dynamic> myMap = Map<String, dynamic>.from(response.body);
        var body = myMap['data'];
